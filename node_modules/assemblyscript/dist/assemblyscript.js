...
`),++this.indentLevel;let F=0;for(let w=Map_keys(L),U=0,W=w.length;U<W;++U){let z=w[U],H=assert(L.get(z));if(H.kind==5){let Q=H,le=this.getExternalCode(Q);(!Du(Q.signature,Lt.IMPORT)||!ke(z)||le)&&(this.makeFunctionImport(v,z,H,le),++F)}else if(H.kind==0){let Q=H;(!Rr(Q.type,Lt.IMPORT)||!ke(z))&&(this.makeGlobalImport(v,z,Q),++F)}}--this.indentLevel,F?(V(s,this.indentLevel),s.push(`}),
